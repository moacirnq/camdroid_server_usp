...
def get_port(id):
    ...
    if cam is None:
        ...
        pipeline = Gst.parse_launch( \
            ...
            #  'avdec_h264 ! theoraenc ! oggmux ! shout2send ip=127.0.0.1 port=8000 password=hackme mount='+ str(cam.id) +'.ogv')
            'v4l2src ! theoraenc ! oggmux ! shout2send ip=127.0.0.1 port=8000 password=hackme mount='+ str(cam.id) + '.ogv')
